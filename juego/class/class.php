...
/*
session_start();
$con=mysql_connect(DB_HOST , SQL_USER , SQL_PASS);
...
function safe($safer)
{
	$a=mysql_real_escape_string(htmlentities($safer,ENT_QUOTES));
	return $a;
}

function safeh($safer)
{
	$a=mysql_real_escape_string(strip_tags($safer,"<br>,<hr>"));
	return $a;
}

function safe_edificio($safer)
{
...
class Conectar
{
	...
	public static function ciudad($id_ciudad)
	{
		$mysqli=new mysqli(DB_HOST,SQL_USER,SQL_PASS,DB_NAME);
		$mysqli->set_charset(DB_CHARSET);
	...
	public static function usuario($id_usuario)
	{
		$mysqli=new mysqli(DB_HOST,SQL_USER,SQL_PASS,DB_NAME);
		$mysqli->set_charset(DB_CHARSET);
	...
	public static function tropa($tropa)
	{
		$mysqli=new mysqli(DB_HOST,SQL_USER,SQL_PASS,DB_NAME);
		$mysqli->set_charset(DB_CHARSET);
	...
	public static function nTropas($id_ciudad)
	{
		$mysqli=new mysqli(DB_HOST,SQL_USER,SQL_PASS,DB_NAME);
		$mysqli->set_charset(DB_CHARSET);
...
class Trabajo
{
	...
	public function registro()
	{
		if (isset($_POST["nombre"]) && isset($_POST["password"]) && isset($_POST["correo"])) //Comprobamos que se ha enviado el formulario
		{
			...
			$res=$this->mysqli->query($sql);
			//************************
			$id_ciudad = Conectar::id_ciudad($id_usuario);
			
			//Le ponemos los edificios básicos
			$sql="insert into edificios_aldea values (null,'Ayuntamiento',0,'ninguno',0,2,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'Granja',0,'cereal',5,1,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'leñador',0,'madera',5,1,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'Barrera',0,'barro',5,1,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'Mina',0,'hierro',5,1,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'Almacen',0,'capacidad',800,2,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'Mercado',0,'comercio',0,4,$id_ciudad)";
			$res=$this->mysqli->query($sql);
			$sql="insert into edificios_aldea values (null,'cuartel',0,'tropas',0,4,$id_ciudad)";
			$res=$this->mysqli->query($sql);
...
class Aldea
{
	...
	public function __construct($requerir=null) //Comprobamos que ha iniciado sesion
	{
		...
		if (isset($_SESSION["ju_nom"]))	//Si esta todo en orden
		{
			...
			$this->id_usuario=Conectar::id($this->usuario);
			...
			$this->ciudad=Conectar::ciudad($this->id_ciudad);
	...
	public function produccion_hora($edificio)	//Muestra la produccion por hora del edificio seleccionado
	{
		...
		if ($edificio == 'granja')
		{
			$nTropas=Conectar::nTropas($this->id_ciudad);
	...
	public function muestra_mercado($comerciantes) //Muestra el mercado
	{
		...
		<?php
		echo "<br />";
		echo "Comerciantes disponibles: $comerciantes_disponibles";
		echo "<br />";
		echo "Recursos que puede transportar cada comerciante: 500";
		echo "<br />";

		if ($comerciantes_disponibles > 0) //Si hay comerciantes disponibles
		{
			?>
			...
			<div id="mercado4">
			<?php
			$this->mostrar_ofertas(); //Mostramos las ofertas
			?>
			</div><!--Mercado4-->
	...
	public function muestra_edificio($edificio) //Para mostrar los datos del edificio que se esta viendo
	{
		...
		else if ($edificio == "mercado") //Si el edificio es el mercado
		{
			if ($reg['nivel']>0) //Si se ha construido el mercado
			{
				...
				<?php
				$this->muestra_mercado($reg['produccion']);
			}
...
//********************************************************************************************************************
	...
	public function mostrar_movimientos_tropas()
	{
		...
		while ($reg=$res->fetch_array())
		{
			echo "<i>".Conectar::ciudad($reg['id_ciudad_atacante'])."</i> ataca a <i>".Conectar::ciudad($reg['id_ciudad_atacada'])."</i>";
			?>
			<br />
			<table>
				<tr>
				...
				{
				?>
					<td><?php echo Conectar::tropa('tropa'.$i);?></td>
				<?php
				}
				?>
				</tr>
		...
		while ($reg=$res->fetch_array())
		{
			echo "<i>".Conectar::ciudad($reg['id_ciudad_atacante'])."</i> refuerza a <i>".Conectar::ciudad($reg['id_ciudad_atacada'])."</i>";
			?>
			<br />
			<table>
				<tr>
	...
	public function accion_tropas($objetivo,$id,$procesar=null)
	{
		if ($objetivo == 'reforzar')
		{
			...
			$sql="insert into tropas_refuerzos values (null,$this->id_ciudad,".$reg['id_ciudad_atacada'].","
		...
		else if ($objetivo == 'atacar')
		{
			...
			else //Si gana el defensor
			{
				...
				$sql="update tropas set tropa1=tropa1-$tropas_restantes[0],tropa2=tropa2-$tropas_restantes[1],
				tropa3=tropa3-$tropas_restantes[2],tropa4=tropa4-$tropas_restantes[3],tropa5=tropa5-$tropas_restantes[4],
	...
	public function mostrar_reportes()
	{
		...
		{
			?>
			<?php echo $reg['objetivo'];?> | <a href="reportes.php?s=1&r=<?php echo $reg['id_reporte'];?>"><?php echo Conectar::ciudad($reg['id_ciudad_atacante'])." ataca a ".Conectar::ciudad($reg['id_ciudad_atacada'])."</a> | ".date("m/d/Y h:i:s",$reg['fecha'])."<br />";?>
			<?php
		}
	}
	...
	public function mostrar_reporte()
	{
		...
		if($reg=$res->fetch_array())
		{
			...
			if ($reg['id_ciudad_atacante']==$this->id_ciudad && $reg['objetivo']=='atacar' && $reg['tropas_atacante']!=$reg['tropasp_atacante'])
			{
				?>
				<table>
					<tr>
						<td></td>
						<td><?php echo Conectar::tropa('tropa1');?></td>
						<td><?php echo Conectar::tropa('tropa2');?></td>
						<td><?php echo Conectar::tropa('tropa3');?></td>
						<td><?php echo Conectar::tropa('tropa4');?></td>
						<td><?php echo Conectar::tropa('tropa5');?></td>
						<td><?php echo Conectar::tropa('tropa6');?></td>
						<td><?php echo Conectar::tropa('tropa7');?></td>
						<td><?php echo Conectar::tropa('tropa8');?></td>
						<td><?php echo Conectar::tropa('tropa9');?></td>
						<td><?php echo Conectar::tropa('tropa10');?></td>
					</tr>
					<tr>
...
//********************************************************************************************************************
	...
	public function mostrar_ofertas() //Muestra las ofertas del mercado
	{
		...
		{
			$n_ciudad=Conectar::ciudad($reg['id_ciudad']); //Muestra el nombre de la ciudad
			?>
...
//********************************************************************************************************************
	...
	public function mostrar_mensajes() //Muestra los mensajes
	{
		...
		{
			echo Conectar::usuario($reg['id_emisor'])." | ".$reg['asunto'];
	...
	public function mostrar_mensaje()
	{
		...
		echo Conectar::usuario($reg['id_emisor']);
		echo "<br />";
		echo str_replace("\n", "<br />", $reg['mensaje']);
		echo "<br />";
	...
	public function formulario_responder_mensaje() //Muestra un formulario para responder un mensaje
	{
		...
		<form name="responder_mensaje" method="post" action="procesa_mensaje.php">
			...
			<?php echo Conectar::usuario($reg['id_emisor']);?><br/>
	...
	{
		$id_destinatario=safen(Conectar::id($_POST['destinatario']));	//ID del usuario al que va dirigido el mensaje
...
//********************************************************************************************************************
	...
	public function mostrar_pefil()
	{
		...
		if (isset($_GET['usuario']))
		{
			...
			$id_usuario=Conectar::id($usuario);
		}
		else
		{
